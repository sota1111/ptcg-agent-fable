"""Deck legality validator (SOT-1794, ported from ptcg-agent-take SOT-1660).

Checks a 60-card deck (a list of card IDs, as stored in ``deck.csv`` /
``decks/**/*.csv``) against the format's construction rules, using the
engine's card database (:func:`cg.api.all_card_data`) as the source of truth
for card identity and type.

Rules enforced (see :func:`validate_deck`):

1. **Size** — exactly 60 cards.
2. **Known cards** — every card ID exists in ``all_card_data()``.
3. **Copy limit** — at most 4 cards sharing the same *name*, EXCEPT Basic
   Energy cards, which may appear any number of times.
4. **Basic Pokémon** — at least 1 Basic Pokémon (you cannot start a game
   without one).
...
Usage::
    ...
    errors = validate_deck(load_deck_csv("decks/candidates/01_dragapult.csv"))
    assert errors == []           # legal deck -> no error strings

Run as a script to validate ``deck.csv`` plus every ``decks/**/*.csv``::

    python3 eval/deck_validator.py [deck.csv ...]
"""
...
def load_deck_csv(path: str) -> List[int]:
    ...
    loader in ``eval/bench.py`` but tolerant of a trailing newline.
    """
    with open(path) as f:
...
def validate_deck(
    ...
    An empty list means the deck is legal. ``registry`` may be injected (e.g.
    in tests) to avoid loading the engine; it defaults to
    :func:`build_registry`.
    """
    if registry is None:
    ...
    if len(card_ids) != DECK_SIZE:
        errors.append(
            f"deck must contain exactly {DECK_SIZE} cards, found {len(card_ids)}")
    ...
    ace_spec = sorted(
        {registry[cid].name for cid in card_ids
         if cid in registry and registry[cid].aceSpec}
    )
    ace_spec_count = sum(
