"""Baseline benchmark runner (SOT-1793, from ptcg-agent-matsu SOT-1671).
...
- fault aggregation — engine rejects (illegal actions), agent exceptions,
  and BaseAgent random-legal fallbacks: all must be 0,
- per-match and per-decision wall-clock timing.

The engine's internal RNG is not externally seedable, so matches vary between
runs; agent seeds are derived per match from --seed for agent-side
reproducibility. For long benches run independent shards (distinct --seed,
--json per shard) and pool them with eval/aggregate_shards.py.

Usage (from the repo root):
    python3 eval/bench.py --agent-a greedy --agent-b random \
        --n 100 --seed 20260720 [--deck deck.csv] [--json out.json]

Agent constructor kwargs can be passed as JSON: --config-a '{"...": ...}'.
"""
import argparse
import json
import math
...
MAX_DECISIONS = 100000  # engine draws long before this
